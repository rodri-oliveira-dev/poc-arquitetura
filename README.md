...
| `PaymentService.Api` / `PaymentService.Worker` | Bounded context de pagamentos externos com API de criacao/consulta, ACL para provider fake/Stripe, persistencia no schema `payment` e Worker estrutural sem webhook, Inbox, Ledger, Balance ou refund nesta etapa. |
