...
- Implementado em etapa posterior: documentação e scripts de obtenção de token (`scripts/validation/get-token.*`) usam Keycloak por `client_credentials` por padrão, preservando fallback para `Auth.Api`.
