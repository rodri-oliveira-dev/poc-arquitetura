...
| `POST /api/v1/payments/{paymentId}/refunds` | `payment.refund` | merchant do Payment persistido deve estar em `merchant_id`. |
...
Solicita refund assincrono para um Payment ja `Completed`. A resposta confirma
que a solicitacao local foi registrada e que a criacao do refund no provider
foi aceita ou ficou em estado pendente; ela nao promete que o estorno interno no
Ledger ja foi concluido.
...
```json
{
  ...
  "externalReference": "refund-pedido-123"
}
```

Para refund total, `amount` pode ser omitido. Refund parcial e rejeitado neste
MVP com `422 Unprocessable Entity`, porque o contrato publico atual do
LedgerService (`POST /api/v1/lancamentos/{lancamentoId}/estornos`) solicita
estorno total do lancamento original e nao possui campo de valor parcial.
...
- Payment deve possuir `ledgerEntryId`;
- Payment ja totalmente `Refunded` e rejeitado;
- Payment com refund pendente incompatibiliza nova solicitacao;
- moeda do refund deve ser a mesma do Payment.

Idempotencia interna:

```text
merchantId + Idempotency-Key + mesmo payload -> replay seguro
merchantId + Idempotency-Key + payload diferente -> 409 Conflict
```

A idempotencia externa usada na Stripe e deterministica:

```text
payment:{paymentId:N}:refund:{refundId:N}:stripe:create-refund
```

O estorno no Ledger usa UUID deterministico:
...
`refund.created` registra/enriquece a referencia externa do refund.
`refund.updated` confirma sucesso quando o status Stripe for `succeeded` e
habilita o estorno no Ledger. `refund.failed` marca falha no provider e nao
solicita estorno interno. `charge.refunded` e conhecido, mas ignorado no MVP
para evitar duplo gatilho financeiro; o fluxo correlacionado usa `refund.*`.
...
`PaymentGateway:Stripe:SecretKey` recebe a API key de teste (`sk_test_...`) usada
...
Para refund, prefira o fluxo correlacionado no sandbox: crie um Payment pelo
PaymentService, confirme o PaymentIntent, aguarde `Completed`, chame
`POST /api/v1/payments/{paymentId}/refunds` e receba os eventos reais
`refund.*` via `stripe listen`. Nao assuma que todos os eventos `refund.*`
podem ser disparados por `stripe trigger` na sua versao da CLI; valide com
`stripe trigger --help` antes de documentar um smoke sintetico.
...
Quando um refund e confirmado pela Stripe, o Worker solicita o estorno no
Ledger pelo contrato publico existente:
...
```json
{
  "motivo": "Payment refund {refundId}: requested_by_customer"
}
```

O retry persistido reutiliza a mesma `Idempotency-Key` deterministica por
`paymentId + refundId + ledger-reversal`. Se o Ledger aceitar a solicitacao e a
resposta HTTP se perder, o retry deve retornar replay idempotente em vez de
criar nova solicitacao logica. Enquanto o Ledger estiver indisponivel, o refund
fica pendente de estorno interno e nao atualiza Balance diretamente.

## Limitacoes atuais

- Nao ha integracao direta com Balance.
- Refund parcial e rejeitado no MVP porque o Ledger ainda nao possui contrato
  publico de estorno parcial.
