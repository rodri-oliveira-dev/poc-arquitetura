...
- Para `terraform plan` real, inicializar antes o backend remoto com `terraform init -backend-config="bucket=rodri-terraform-state-bucket"` e nao usar `-lock=false`.
