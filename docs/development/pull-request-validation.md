...
O workflow `main-dotnet-ci` permanece como validacao completa de `push` na `main`, `pull_request` para `main` e execucao manual, incluindo cobertura, SonarQube Cloud e relatorios. Durante a migracao para SonarQube Cloud contextual, PRs executam somente os contextos Sonar impactados por paths; o projeto global fica restrito a `push` na `main` e `workflow_dispatch` com `sonar_context=all` para comparacao de baseline.
