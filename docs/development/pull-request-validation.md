...
O workflow `main-dotnet-ci` permanece como validacao completa de `push` na `main`, `pull_request` para `main` e execucao manual, incluindo cobertura, SonarQube Cloud e relatorios. Durante a migracao para SonarQube Cloud contextual, PRs executam temporariamente os seis contextos Sonar sem selecao por paths; o projeto global fica restrito a `push` na `main` e `workflow_dispatch` para comparacao de baseline.
...
| `main-dotnet-ci` | `.github/workflows/dotnet.yml` | `push` na `main`, `pull_request` para `main`, `workflow_dispatch` | Validacao completa com SonarQube Cloud contextual para Ledger, Balance, Transfer, Identity, Audit e Shared, artifacts independentes e summary consolidado. O job global da solution agregadora permanece temporariamente em `push` na `main` e `workflow_dispatch`, com gate de cobertura global de 85%, para comparacao durante a migracao. Em PR, os seis contextos ainda rodam ate a selecao por paths da proxima etapa. | Bloqueante se exigido por branch protection/ruleset |
