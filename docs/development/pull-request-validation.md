...
| `infrastructure-security` | `.github/workflows/infrastructure-security.yml` | `pull_request` e `push` para `main` quando ha mudancas em `infra/terraform/**`, Dockerfiles, Compose, `.github/actions/trivy-repository-scan/**` ou no proprio workflow; `workflow_dispatch` | Executa Trivy para Dockerfile, Compose, Terraform, misconfigurations, secrets e filesystem. Nao instala Terraform/TFLint, nao executa `plan`, `apply` nem publica artifacts externos. | Bloqueante se exigido por branch protection/ruleset |
| `terraform-validation` | `.github/workflows/terraform-validation.yml` | `pull_request` e `push` para `main` quando ha mudancas em `infra/terraform/**`, scripts de validacao Terraform ou no proprio workflow; `workflow_dispatch` | Executa `fmt -check`, `init -backend=false`, `validate` e TFLint. Nao executa `plan`, `apply` nem `destroy`; o `init` sem backend e apenas validacao sintatica sem credenciais. Plan real no CI deve inicializar o backend remoto GCS e nao usar `-lock=false`. | Bloqueante se exigido por branch protection/ruleset |
| `container-baseline` | `.github/workflows/container-baseline.yml` | `pull_request` e `push` para `main` quando ha mudancas em Compose, Dockerfiles, `.dockerignore`, `global.json`, props centrais, `.csproj`, validador, skill ou workflow relacionado; `workflow_dispatch` | Executa `docker compose config --quiet`, validador estrutural, self-test negativo controlado do validador e `docker compose build` da stack base. Nao faz login, push ou publicacao em registry. | Bloqueante se exigido por branch protection/ruleset |
