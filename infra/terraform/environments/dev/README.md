...
Cloud SQL dev uses a disposable, cost-conscious ZONAL PostgreSQL instance by
default with the `ENTERPRISE` edition, `db-f1-micro` tier, 10 GB disk,
`disk_autoresize=false`, `deletion_protection=false`, backups disabled, and
point-in-time recovery disabled. Public IPv4 is enabled only to support local
access through Cloud SQL Auth Proxy in this first iteration. The module does
not configure `authorized_networks`, does not allow `0.0.0.0/0`, does not
create Secret Manager resources, and never exposes the database password as an
output.
