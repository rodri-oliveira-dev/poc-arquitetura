...
- `AGENTS.md`: regras globais para agentes de código, incluindo multitenancy obrigatório.
- `.agents/skills/`: skills reutilizáveis para DDD, .NET, EF Core, testes, cobertura, observabilidade, containers, CI e multitenancy.
- `docs/adrs/`: decisões arquiteturais iniciais, incluindo a origem do tenant pela claim `tenant_id` e o isolamento lógico por linha.
...
4. Preserve a decisão multitenant registrada na ADR-0001.
5. Ajuste o nome e a descrição do projeto no `AGENTS.md`, sem remover suas regras de isolamento.
6. Acrescente as versões necessárias ao `Directory.Packages.props` conforme os pacotes forem introduzidos.
...
Desde a primeira fatia, crie dados para pelo menos dois tenants e valide que não existe leitura ou alteração cruzada.
