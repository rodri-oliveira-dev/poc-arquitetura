...
case "$PROVIDER" in
  ...
  PubSub|pubsub)
    echo "Os testes de carga padrao usam Kafka. Pub/Sub e legado/opt-in para a stack local; nao ha runner k6 Pub/Sub versionado. Use --provider Kafka ou suba ./scripts/local/start-stack-pubsub.sh para validacoes manuais legadas." >&2
    exit 2
    ;;
  *)
...
mapfile -t compose_env_args < <(print_compose_env_args)
...
kafka_topic_end_offset() {
  ...
  if ! output="$(docker compose "${compose_env_args[@]}" "${compose_files[@]}" exec -T kafka \
    ...
    echo "Falha ao consultar offset Kafka do topico $topic. Se o topico estiver ausente, rode ./scripts/local/start-stack.sh para executar kafka-init-topics. Detalhe: $output" >&2
    exit 1
  fi
...
find_transfer_kafka_event() {
  ...
  echo "Kafka nao encontrou $event_type com correlationId=$correlation_id no topico $topic. Se o topico estiver ausente, rode ./scripts/local/start-stack.sh para executar kafka-init-topics." >&2
  exit 1
}
...
# a) gerar env
COMPOSE_FILE="$COMPOSE_FILE" OUT_FILE="$ENV_FILE" "$ROOT_DIR/scripts/lib/compose-env.sh" >/dev/null
...
for _ in $(seq 1 30); do
  if TOKEN="$(ENV_FILE="$ENV_FILE" "$ROOT_DIR/scripts/validation/get-token.sh" 2>/dev/null)" && [[ -n "$TOKEN" ]]; then
    break
  fi
  sleep 2
done
...
if [[ -z "$TOKEN" ]]; then
  TOKEN="$(ENV_FILE="$ENV_FILE" "$ROOT_DIR/scripts/validation/get-token.sh")"
fi
if [[ -z "$TOKEN" ]]; then
  echo "Falha ao obter TOKEN. Voce pode informar manualmente via env TOKEN=..." 1>&2
  exit 1
fi
...
run_k6() {
  ...
  docker compose "${compose_env_args[@]}" -f "$COMPOSE_FILE" -f "$COMPOSE_K6_FILE" --profile k6 run --rm \
    ...
    "$@" \
    k6 run "$script_path" --summary-export "/artifacts/$summary_file"

  if [[ ! -f "$host_summary" ]]; then
    echo "Summary k6 nao encontrado: $host_summary" >&2
    exit 1
  fi
