...
function Start-LocalStackForZap {
  if ($UseNginx) {
    throw "-StartStack sobe apenas a stack local direta. Para Nginx, execute ./scripts/local/start-full-stack.ps1 antes e rode este script com -UseNginx."
  }

  $startScript = Join-Path $root "scripts\local\start-stack.ps1"
...
function Invoke-HealthCheck([string]$ApiName, [string]$BaseUrl) {
  ...
  $suggestion = if ($UseNginx) {
    "Suba a stack completa com Nginx, por exemplo ./scripts/local/start-full-stack.ps1, e confirme os certificados locais."
  } else {
    "Suba a stack local, por exemplo ./scripts/local/start-stack.ps1, ou execute este script com -StartStack."
  }
...
function Write-Summary {
  ...
  if ($UseAuthentication) {
    $lines.Add("- Authorization Bearer foi injetado via ZAP Replacer usando token obtido por ``scripts/validation/get-token.ps1`` ou pelo parametro ``-Token``.")
  }
