...
using TransferService.Api.Options;
...
public static class JwtAuthServiceCollectionExtensions
{
        ...
        IHostEnvironment environment)
    {
        ...
        if (string.IsNullOrWhiteSpace(jwtOptions.Issuer))
            throw new InvalidOperationException($"{JwtAuthOptions.SectionName}:Issuer e obrigatorio.");

        if (string.IsNullOrWhiteSpace(jwtOptions.Audience))
            throw new InvalidOperationException($"{JwtAuthOptions.SectionName}:Audience e obrigatorio.");

        if (string.IsNullOrWhiteSpace(jwtOptions.JwksUrl))
            throw new InvalidOperationException($"{JwtAuthOptions.SectionName}:JwksUrl e obrigatorio.");
    ...
    private static void ValidateTransportSecurity(JwtAuthOptions options, IHostEnvironment environment)
    {
        ...
        if (!options.RequireHttpsMetadata)
            throw new InvalidOperationException($"{JwtAuthOptions.SectionName}:RequireHttpsMetadata=false e permitido apenas em Development/Local.");
    ...
    private sealed class JwksOnlyConfigurationRetriever : IConfigurationRetriever<OpenIdConnectConfiguration>
    {
        ...
        private static async Task<OpenIdConnectConfiguration> GetAsync(
            string jwksAddress,
            IDocumentRetriever retriever,
            CancellationToken cancel)
        {
