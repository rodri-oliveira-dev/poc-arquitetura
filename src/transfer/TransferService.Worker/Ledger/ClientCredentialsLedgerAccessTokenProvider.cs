...
public sealed class ClientCredentialsLedgerAccessTokenProvider : ILedgerAccessTokenProvider, IDisposable
{
    ...
    private readonly IOptionsMonitor<TransferWorkerOptions> _options;
    ...
    public ClientCredentialsLedgerAccessTokenProvider(
        ...
        IOptionsMonitor<TransferWorkerOptions> options,
    ...
    {
        var options = _options.CurrentValue.Ledger.Auth;
    ...
    private async Task<CachedAccessToken> RequestTokenAsync(
        LedgerClientAuthOptions options,
        CancellationToken cancellationToken)
    {
        if (options.TokenEndpoint is null)
        {
            throw new LedgerAuthenticationException("TransferService.Worker nao possui TokenEndpoint configurado para autenticar no LedgerService.Api.");
        }
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new LedgerAuthenticationException(
                $"Falha ao obter token para LedgerService.Api: HTTP {(int)response.StatusCode} ({response.StatusCode}). Resposta: {body}");
        }

        var token = await response.Content.ReadFromJsonAsync<TokenResponse>(cancellationToken)
            ?? throw new LedgerAuthenticationException("Keycloak retornou resposta vazia ao emitir token para LedgerService.Api.");

        if (string.IsNullOrWhiteSpace(token.AccessToken))
        {
            throw new LedgerAuthenticationException("Keycloak nao retornou access_token ao emitir token para LedgerService.Api.");
        }
    ...
    private static IEnumerable<KeyValuePair<string, string>> BuildTokenRequest(LedgerClientAuthOptions options)
    {
